...
void Testbench::feed_rgb() {
  sc_dt::sc_uint<9> x, y; // for loop counter
  ...
  for (y = 0; y != height + 1; ++y) {
    for (x = 0; x != width + 1; ++x) {
      if (y >= 1 && x >= 1) {
          ...
          tmp = i_result.read();
        #endif
        *(target_bitmap + bytes_per_pixel * (width * (y - 1) + (x - 1)) + 2) = tmp.range(7, 0);
        *(target_bitmap + bytes_per_pixel * (width * (y - 1) + (x - 1)) + 1) = tmp.range(15, 8);
        *(target_bitmap + bytes_per_pixel * (width * (y - 1) + (x - 1)) + 0) = tmp.range(23, 16);
      }
    }
  }

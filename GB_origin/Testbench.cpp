...
void Testbench::fetch_result() {
  ...
  for (y = 0; y != height; ++y) {
    for (x = 0; x != width; ++x) {
			...
			tmp = i_result.read();
#endif
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = tmp.range(7, 0);
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = tmp.range(15, 8);
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = tmp.range(23, 16);
    }
  }
